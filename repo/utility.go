...
func SelectRows[T any](ctx context.Context, db *db.DB, builder sq.SelectBuilder, scanFn pgx.RowToFunc[T]) ([]T, error) {
	fmt.Println("Inside SelectRows")

	sql, args, err := builder.ToSql()
	if err != nil {
		//fmt.Println("Error in building sql at Select Rows:", err.Error())
	...
	rows, err := db.Query(ctx, sql, args...)
	if err != nil {
		//fmt.Println("Error in running query at Select Rows:", err.Error())
	...
	if err != nil {
		//fmt.Println("Error in collecting rows at Select Rows:", err.Error())
		//log.Error("pgutility, err collecting Rows at Select Rows :", err.Error())
		return nil, err
	}
